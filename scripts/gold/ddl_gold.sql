...
-- If the view already exists, drop it to recreate
DROP VIEW IF EXISTS gold.dim_customers;

-- Create a new view for the customer dimension in the gold layer
...
SELECT
    ROW_NUMBER() OVER (ORDER BY pn.prd_id, pn.prd_start_dt, pn.prd_key) AS product_key,
    
    pn.prd_id AS product_id,
    pn.prd_key AS product_number,
    pn.prd_nm AS product_name,

    -- Extract cat_id from prd_key (first 5 chars, replacing dash)
